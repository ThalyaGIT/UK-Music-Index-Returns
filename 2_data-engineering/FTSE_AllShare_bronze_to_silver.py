...
def main(days, effect_days, bronze_data_folder, silver_data_folder):   
    ...
    df = pd.read_csv(FTSAllShare_file)
    ...
    df['% FTSEAllShare Change'] = ((df['Price'] - df['Previous Price']) / df['Previous Price']) * 100
    ...
    output_file = os.path.join(silver_data_folder, 'FTSEAllShare.csv')
