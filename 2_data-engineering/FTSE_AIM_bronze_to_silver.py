...
def main(days, effect_days, bronze_data_folder, silver_data_folder):   
    ...
    FTSAIM_file = os.path.join(bronze_data_folder, 'downloaded_FTSEAIM.csv')

    # Load the CSV file into a pandas DataFrame
    df = pd.read_csv(FTSAIM_file)
    ...
    df['% FTSEAIM Change'] = ((df['Price'] - df['Previous Price']) / df['Previous Price']) * 100

    # Round up % FTSEAIM Change to 2 decimal places
    df['% FTSEAIM Change'] = df['% FTSEAIM Change'].round(2)

    df['Previous % FTSEAIM Change'] = df['% FTSEAIM Change'].shift(days)

    df['Next % FTSEAIM Change'] = df['% FTSEAIM Change'].shift(-effect_days)

    # Keep only relevant columns
    result_df = df[['% FTSEAIM Change', 'Previous % FTSEAIM Change', 'Next % FTSEAIM Change']]
    ...
    output_file = os.path.join(silver_data_folder, 'FTSEAIM.csv')

    # Save the new DataFrame to a CSV file in the "silver" folder
    result_df.to_csv(output_file, index=False)

    # Display message
    print('___FTSE AIM data processed and saved to silver layer')
