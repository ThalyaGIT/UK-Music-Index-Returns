...
result_df['% MSCI UK Change'] = ((result_df['This Week\'s End of Week Closing'] - result_df['Previous End of Week Closing']) / result_df['Previous End of Week Closing']) * 100

# Round up % FTSE100 Change to 2 decimal places
result_df['% MSCI UK Change'] = result_df['% MSCI UK Change'].apply(lambda x: round(x, 2))

# Calculate the previous week's % FTSE100 Change
result_df['Previous Week % MSCI UK Change'] = result_df['% MSCI UK Change'].shift(1)

# Keep only relevant Columns
result_df = result_df[['End of Week Date', '% MSCI UK Change']]
