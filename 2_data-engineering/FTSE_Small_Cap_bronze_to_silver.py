...
df['% FTSESmallCap Change'] = ((df['Price'] - df['Previous Week Price']) / df['Previous Week Price']) * 100

# Round up % FTSEAllShare Change to 2 decimal places
df['% FTSESmallCap Change'] = df['% FTSESmallCap Change'].round(2)

df['Previous Week % FTSESmallCap Change'] = df['% FTSESmallCap Change'].shift(shift)
...
# Display message
print('FTSESmallCap data processed and saved to silver layer')