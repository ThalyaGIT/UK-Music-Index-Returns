...
csv_file = os.path.join(data_folder, 'downloaded_FTSE_Small_Cap.csv')
...
result_df['% FTSE Small Cap Change'] = ((result_df['This Week\'s End of Week Closing'] - result_df['Previous End of Week Closing']) / result_df['Previous End of Week Closing']) * 100

# Round up % FTSE100 Change to 2 decimal places
result_df['% FTSE Small Cap Change'] = result_df['% FTSE Small Cap Change'].apply(lambda x: round(x, 2))

# Calculate the previous week's % FTSE100 Change
result_df['Previous Week % FTSE Small Cap Change'] = result_df['% FTSE Small Cap Change'].shift(1)

# Keep only relevant Columns
result_df = result_df[['End of Week Date', '% FTSE Small Cap Change']]
...
output_file = os.path.join(silver_folder, 'FTSESmallCap.csv')
