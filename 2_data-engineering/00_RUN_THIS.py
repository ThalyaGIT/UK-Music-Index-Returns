...
def run_script(script_name, days, bronze_data_folder, silver_data_folder):
    script_path = os.path.join(script_folder, script_name)
    result = subprocess.run(['python', script_path, str(days), bronze_data_folder, silver_data_folder])
    if result.returncode != 0:
        print(f'{script_name} failed with return code {result.returncode}')
    else:
        print(f'{script_name} completed successfully with days={days}')

def run_all_scripts(scripts, days_list, bronze_data_folder, silver_data_folder):
    for days in days_list:
        for script in scripts:
            run_script(script, days, bronze_data_folder, silver_data_folder)

# List of days to loop over
days_list = [1, 3, 5, 10, 20]

run_all_scripts(scripts, days_list, bronze_data_folder, silver_data_folder)